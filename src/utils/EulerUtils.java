...
public class EulerUtils {
	...
	public static final Function<Stream<BigInteger>, Function<Function<BigInteger, Boolean>, BigInteger>> fibonacciWithStop = fibs -> stopCondition -> {
		Optional<BigInteger> result = fibs.filter(f -> stopCondition.apply(f)).findFirst();
		return result.orElse(BigInteger.ZERO);
	};

	public static final Function<Stream<Entry<Integer, BigInteger>>, Function<Function<BigInteger, Boolean>, Integer>> fibonacciWithStopIdx = fibs -> stopCondition -> fibs
			.filter(f -> stopCondition.apply(f.getValue()))
			// .peek(System.out::println)
			.map(e -> e.getKey()).findFirst().orElse(-1);

	public static final Supplier<Stream<BigInteger>> fibonacci = () -> {
		Stream<BigInteger> result = Stream.iterate(new BigInteger[] { BigInteger.ONE, BigInteger.ONE },
				p -> new BigInteger[] { p[1], p[0].add(p[1]) }).map(p -> p[1]);
		return result;
	};
	...
	public static List<Integer> divisors(final Integer number) {
		...
		Stream<Tuple<Integer, Integer>> results = Stream.iterate(1, n -> n + 1).limit(half).filter(i -> number % i == 0)
	...
	private static TailCall<BigInteger> expBySquaring(BigInteger y, BigInteger x, BigInteger n) {
		if (n.compareTo(BigInteger.ZERO) < 0) { // n < 0
			...
			return call(() -> expBySquaring(y.multiply(x), x.multiply(x),
					(n.subtract(BigInteger.ONE)).divide(BigInteger.valueOf(2))));
		} else {
	...
	public static Map<BigInteger, BigInteger> factorialTable(int n) {
		Map<BigInteger, BigInteger> factorials = Stream
				.iterate(new Tuple<>(BigInteger.ONE, BigInteger.ONE),
						x -> new Tuple<>(x.first.add(BigInteger.ONE), x.second.multiply(x.first.add(BigInteger.ONE))))
				.limit(n).collect(Collectors.toMap(x -> x.first, x -> x.second, (a, b) -> {
	 ...
	 *
	 * @param <T>
	 *            A Node class to implement directed graphs (or trees)
	 */
	 ...
	 * two-arguments function to the elements of this stream. The first argument of
	 * the function is the element index and the second one - the element value.
	 */
	...
	public static <T, U, R> Function<T, Stream<R>> crossJoin(Supplier<Stream<? extends U>> streamSupplier,
			BiFunction<? super T, ? super U, ? extends R> combiner) {
		return t -> streamSupplier.get().map(u -> combiner.apply(t, u));
	}

	public static BinaryOperator<Function<String, Stream<String>>> kleisli = (f, g) -> s -> f.apply(s).flatMap(g);

	/**
	 * http://sebastian-millies.blogspot.com/2015/09/cartesian-products-with-kleisli.html
	 * 
	 * @param n
	 * @param coll
	 * @return
	 */
	public static List<String> cartesian(int n, Collection<String> coll) {
		return coll.stream().flatMap(IntStream.range(1, n).boxed().map(_any -> crossJoin(coll::stream, String::concat)) // create
																														// (n-1)
																														// appropriate
																														// crossJoin
																														// instances
	...
	public static BiFunction<Character, Character, List<String>> alphabet = (lowerBound, upperBound) -> {
		return IntStream.rangeClosed(lowerBound, upperBound).mapToObj(c -> String.valueOf((char) c))
				.collect(Collectors.toList());
	};
